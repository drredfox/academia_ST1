...
_rifleDetails = [_primaryWeapon, ASORGS_DB select DB_Rifles] call ASORGS_fnc_GetDetails;
_launcherDetails = [_launcher, ASORGS_DB select DB_Launchers] call ASORGS_fnc_GetDetails;
_pistolDetails = [_handgun, ASORGS_DB select DB_Handguns] call ASORGS_fnc_GetDetails;
...

{
	...
	if(_magClass in _rifleMags) then {
		_playerRifleAmmo set [count _playerRifleAmmo, _magClass];
		_placed = true;
	};
	if((_magClass in _launcherMags) && !_placed) then {
		_playerLauncherAmmo set [count _playerLauncherAmmo, _magClass];
		_placed = true;
	};
	if((_magClass in _pistolMags) && !_placed) then {
		_playerPistolAmmo set [count _playerPistolAmmo, _magClass];
		_placed = true;
	};
	if(!_placed) then {
		if(!(_magClass in ASORGS_throwable)) then {
			if(!(_magClass in ASORGS_explosives)) then {
				_playerExtraAmmo set [count _playerExtraAmmo, _magClass];
			};
		};
	};
...
#define COMBO_ADD(ITEMNAME, ITEMCLASS, ITEMID, ITEMPIC) \
				...
				_comboControl lbSetData [(lbSize _comboControl)-1,ITEMCLASS]; \
				_comboControl lbSetValue [(lbSize _comboControl)-1,ITEMID]; \
				_comboControl lbSetPicture [(lbSize _comboControl)-1,ITEMPIC]; \
				if(_currentMag == ITEMCLASS) then { _comboControl lbSetCurSel ((lbSize _comboControl)-1);	};
	...
	if(count _rifleDetails > 0) then {
		{
			...
			if((count _details > 0) && ([(_details select DBF_Class), false] call ASORGS_fnc_IsAllowed) && !(_class in _alreadySelected)) then
			{
				...
				if(_class == _currentMag) then {
					...
					_alreadySelected = _alreadySelected + [_currentMag];
				};
			};
