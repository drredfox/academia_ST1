...
hyp_fnc_traceFireOff = {
	  private["_this","_unit","_color","_lifetime","_interval","_maxDistance","_maxDuration","_eventHandle"];
	  _unit = _this select 0;
	  if(_unit in hyp_var_tracer_tracedUnits) then {
			[_unit] spawn hyp_fnc_traceFireClear;
			_unit removeEventHandler ["fired", (_unit getVariable "hyp_var_tracer_eventHandle")];
			_unit setVariable ["hyp_var_tracer_eventHandle", nil];
			hyp_var_tracer_tracedUnits = hyp_var_tracer_tracedUnits - [_unit];
	  };
};
